#ifndef LIST_STACK_ALLOCATOR_H
#define LIST_STACK_ALLOCATOR_H
...
public:
    ...
    iterator emplace(const_iterator iterator, Args&&... args) {
        base_node_pointer ptr = insert_(iterator.node, std::forward<Args>(args)...);
        return ptr;
    }

    template <typename... Args>
    void emplace_front(Args&&... args) {
        insert_(begin().node, std::forward<Args>(args)...);
    }

    template <typename... Args>
    void emplace_back(Args&&... args) {
        insert_(end().node, std::forward<Args>(args)...);
    }

    // inserts value before iterator position
    iterator insert(const_iterator iterator, const value_type& value) {
        base_node_pointer ptr = insert_(iterator.node, value);
        return ptr;
    }

    void push_front(const value_type& value) {
        insert_(begin().node, value);
    }

    void push_back(const value_type& value) {
        insert_(end().node, value);
    }

    // inserts value before iterator position
    iterator insert(const_iterator iterator, value_type&& value) {
        base_node_pointer ptr = insert_(iterator.node, std::move(value));
        return ptr;
    }

    void push_front(value_type&& value) {
        insert_(begin().node, std::move(value));
    }

    void push_back(value_type&& value) {
        insert_(end().node, std::move(value));
    }
...
private:
    ...
    template <typename... Args>
    base_node_pointer insert_(base_node_pointer node, Args&&... args) {
